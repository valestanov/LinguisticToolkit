...
class Lemma:
    def __init__(self, original: str, for_match: str = None, in_dict: bool = None, is_lang: bool = None, properties = None, dict_content = None, pos: int = None):
        ...
        self.pos = pos  # 单词在句子中的位置，对于扩展字符按照实际字符单位计算，如ExtB一个字为2。
        self.char_pos = self.pos #实际的单位数量
        self.isFocus = False #是否当前光标聚焦
        self.length = self.len()
    
    def len(self):
        word_len = len(self.original)
        word_len += len([char for char in self.original if ord(char)> 0xFFFF]) #扩展字符
        return word_len
...
class SentenceSplitter:
    ...
    def _update_positions(self):
        ...
        for word in self.word_list:
            ...
            # 原为current_pos += len(word.original)
    ...
    def compare_vocabulary(self, vocabulary: List[str]):
        ...
        sorted_vocab = [entry for entry in sorted_vocab if entry in full_match_string]
    ...
    def set_word_focus(self, pos: int) -> int:
        for word in self.word_list:
            if word.pos <= pos < word.pos + word.length:
                word.isFocus = True
            else:
                word.isFocus = False
